...
int compare_regions(ulv *bufa, ulv *bufb, size_t count) {
    ...
    for (i = 0; i < count; i++, p1++, p2++) {
        if (*p1 != *p2) {
            fprintf(stderr, "FAILURE: 0x%08lx != 0x%08lx at bufa:0x%08lx bufb:0x%08lx  p1:0x%08lx p2:0x%08lx offset 0x%08lx.\n", 
                (ul) *p1, (ul) *p2,(ul) bufa,(ul) bufb,(ul) p1,(ul) p2, (ul) i);
    ...
    for (j = 0; j < 16; j++) {
        ...
        for (i = 0; i < count; i++, p1++) {
            if (*p1 != (((j + i) % 2) == 0 ? (ul) p1 : ~((ul) p1))) {
                fprintf(stderr, 
                    "FAILURE: possible bad address line at base:0x%08lx  offset: 0x%08lx.\n", 
                    (ul) bufa,(ul) i);
                printf("Skipping to next test...\n");
                fflush(stdout);
                return -1;
